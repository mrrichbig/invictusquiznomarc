...
import FloatingVSL from "@/components/floating-vsl"
...
export default function InvictusQuiz() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("[v0] Dados do formulário (somente log):", formData)
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-cyber-gradient relative overflow-hidden pt-8`}>
        ...
        <div
          className={`container mx-auto px-4 ${isMobile ? "py-4" : "py-8"} ${isMobile ? "max-w-full" : "max-w-4xl"}`}
        >
          <Card
          ...
          >
            ...
            <CardContent className={`${isMobile ? "p-6" : "p-12"} relative z-10`}>
              <form onSubmit={handleSubmit} className={`space-y-${isMobile ? "8" : "10"}`}>
                <div className="text-center mb-8 animate-cyber-scale-in">
                  <div className="inline-block relative mb-6">
                    <div className="absolute -inset-4 bg-cyber-red/20 rounded-full blur-xl animate-cyber-glow-pulse"></div>
                    <Crown
                      className={`${isMobile ? "h-12 w-12" : "h-16 w-16"} text-cyber-red mx-auto relative z-10 cyber-text-glow`}
                    />
                  </div>

                  <h1
                    className={`${isMobile ? "text-2xl" : "text-4xl"} font-modern font-bold text-cyber-gray-100 mb-4 cyber-text-glow`}
                  >
                    AVALIAÇÃO EXCLUSIVA
                  </h1>

                  <p
                    className={`${isMobile ? "text-sm" : "text-lg"} text-cyber-gray-300 font-modern max-w-2xl mx-auto`}
                  >
                    Apenas 3% dos que chegam até aqui são aprovados. Suas respostas determinarão se você faz parte da
                    elite.
                  </p>
                </div>
                ...
                <div className={`space-y-${isMobile ? "6" : "8"}`}>
                  ...
                  >
                    ...
                    />
                    <span className={`${isMobile ? "text-base" : ""}`}>Seus Dados</span>
                  </h2>
                ...
                <div className={`space-y-${isMobile ? "8" : "10"}`}>
                  ...
                  >
                    <Brain
                      className={`${isMobile ? "h-5 w-5" : "h-6 w-6"} text-red-500 cyber-text-glow animate-cyber-glow-pulse flex-shrink-0`}
                    />
                    <span className={`${isMobile ? "text-base" : ""}`}>Teste de Mentalidade</span>
                  </h2>
                  ...
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-600">
                    ...
                    >
                      1. Qual mentalidade define você hoje?
                    </Label>
                    <RadioGroup
                      value={formData.mentalidade}
                      onValueChange={(value) => setFormData((prev) => ({ ...prev, mentalidade: value }))}
                      className="space-y-3"
                    >
                      {[
                        "Ainda busco minha identidade e propósito",
                        "Sei quem sou, mas preciso de direção",
                        "Tenho clareza do meu caminho, falta estratégia",
                        "Sou imparável, só preciso de aceleração",
                      ].map((option, index) => (
                        <div
                          key={option}
                  ...
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-700">
                    <Label
                      className={`${isMobile ? "text-base" : "text-xl"} font-modern font-semibold text-cyber-gray-100`}
                    >
                      2. Qual o seu maior propósito na vida?
                    </Label>
                    <RadioGroup
                      value={formData.propositoVida}
                      onValueChange={(value) => setFormData((prev) => ({ ...prev, propositoVida: value }))}
                      className="space-y-3"
                    >
                      {[
                        "Liberdade financeira para mim e minha família",
                        "Construir um legado que transcenda gerações",
                        "Impactar vidas e transformar realidades",
                        "Dominar meu mercado e ser referência",
                      ].map((option, index) => (
                        <div
                          key={option}
                          className={`flex items-center space-x-4 ${isMobile ? "p-4" : "p-5"} rounded-lg border-cyber-dark hover:border-cyber bg-cyber-card/40 hover:bg-cyber-card/60 transition-all duration-300 backdrop-blur-sm hover:cyber-glow hover-cyber-lift cursor-pointer animate-cyber-slide-in`}
                  ...
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-800">
                    <Label
                      className={`${isMobile ? "text-base" : "text-xl"} font-modern font-semibold text-cyber-gray-100`}
                    >
                      3. O que você está disposto a sacrificar pelo seu sucesso?
                    </Label>
                    <RadioGroup
                      value={formData.sacrificio}
                      onValueChange={(value) => setFormData((prev) => ({ ...prev, sacrificio: value }))}
                      className="space-y-3"
                    >
                      {[
                        "Conforto temporário por resultados permanentes",
                        "Entretenimento e lazer em troca de construção",
                        "Aprovação social para seguir meu próprio caminho",
                        "Tudo que for necessário para alcançar a elite",
                      ].map((option, index) => (
                        <div
                          key={option}
                          className={`flex items-center space-x-4 ${isMobile ? "p-4" : "p-5"} rounded-lg border-cyber-dark hover:border-cyber bg-cyber-card/40 hover:bg-cyber-card/60 transition-all duration-300 backdrop-blur-sm hover:cyber-glow hover-cyber-lift cursor-pointer animate-cyber-slide-in`}
                  ...
                  {/* Question 4 */}
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-900">
                    <Label
                      className={`${isMobile ? "text-base" : "text-xl"} font-modern font-semibold text-cyber-gray-100`}
                    >
                      4. Se tivesse que escolher agora, qual decisão define seu futuro?
                    </Label>
                    <RadioGroup
                      value={formData.decisao}
                      onValueChange={(value) => setFormData((prev) => ({ ...prev, decisao: value }))}
                      className="space-y-3"
                    >
                      {[
                        "Continuar na zona de conforto e aceitar a mediocridade",
                        "Tentar mais uma vez por conta própria, sem mentoria",
                        "Investir em mim mesmo e buscar quem já venceu",
                        "Fazer parte de uma elite que me force a evoluir",
                      ].map((option, index) => (
                        <div
                          key={option}
                          className={`flex items-center space-x-4 ${isMobile ? "p-4" : "p-5"} rounded-lg border-cyber-dark hover:border-cyber bg-cyber-card/40 hover:bg-cyber-card/60 transition-all duration-300 backdrop-blur-sm hover:cyber-glow hover-cyber-lift cursor-pointer animate-cyber-slide-in`}
                  ...
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-700">
                    <Label
                      htmlFor="diferencial"
                      className={`${isMobile ? "text-base" : "text-xl"} font-modern font-semibold text-cyber-gray-100 flex items-center gap-2`}
                    >
                      <Target
                        className={`${isMobile ? "h-5 w-5" : "h-6 w-6"} text-red-500 cyber-text-glow flex-shrink-0`}
                      />
                      5. O que te torna diferente dos 97% que desistem?
                    </Label>
                    <Textarea
                      id="diferencial"
                      ...
                      placeholder="Seja genuíno. Queremos saber quem você realmente é..."
                    />
                  </div>

                  {/* Question 6 - Open Text */}
                  <div className="space-y-4 animate-cyber-fade-up delay-cyber-800">
                    <Label
                      htmlFor="visaoFuturo"
                      className={`${isMobile ? "text-base" : "text-xl"} font-modern font-semibold text-cyber-gray-100`}
                    >
                      6. Onde você se vê daqui a 12 meses com acesso à Ordem Invictus?
                    </Label>
                    <Textarea
                      id="visaoFuturo"
                      value={formData.visaoFuturo}
                      onChange={(e) => setFormData((prev) => ({ ...prev, visaoFuturo: e.target.value }))}
                      className={`${isMobile ? "min-h-[120px] text-sm" : "min-h-[140px] text-lg"} border-cyber focus:border-cyber-light bg-cyber-card text-cyber-gray-100 placeholder:text-cyber-gray-500 transition-all duration-300 font-modern rounded-lg leading-relaxed backdrop-blur-sm focus:cyber-glow hover-cyber-border focus-cyber`}
                      placeholder="Descreva sua visão de futuro com detalhes..."
                    />
                  </div>
                </div>
